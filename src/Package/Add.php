...
class Add
{
    public static function main(OutputInterface $output, array $data = [], $update = false): void
    {
        ...
        $appLockJson = [
            "_readme" => [
                ...
                "Read more about it at " . ConfigService::getDocumentationUrl()
            ]
        ];
        ...
        if (!$update) {
            foreach ($data as $name => $ver) {
                $dependencies->put($name, $ver ?: 'latest');
            }
        } else {
        ...
        // 获取云端包
        $cloudPackages = collect(Package::query($queryData->toArray()));
